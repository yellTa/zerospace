...
public class CalendarServiceImpl {
    ...
    public ResponseEntity<?> getCalendarInfo(HttpServletRequest request) {
        ...
        String hourplaceEmail = hourplaceAccount.getHourplaceEmail();
        String hourplacePassword = hourplaceAccount.getHourplacePassword();

        WebDriver driver = crawlingLogic.loginCheck("hourplace", userId);
        if(driver ==null){
        ...
        SpacecloudAccount spacecloudAccount = spacecloudAccountRepository.findByuserId(userId);
        String spacecloudEmail = spacecloudAccount.getHourplaceEmail();
        String spacecloudPassword = spacecloudAccount.getHourplacePassword();
        // 로그인이 맞으면 크롤링 비교하기
